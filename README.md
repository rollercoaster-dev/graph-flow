...
File-based workflow checkpoint, knowledge management, and code graph analysis for Claude Code and other MCP-capable hosts.
...
# Initialize for a project (writes .mcp.json and indexes code/docs)
graph-flow init
...
Or use the project-local `.mcp.json` generated by `graph-flow init`.
