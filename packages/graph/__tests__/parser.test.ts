...
describe("CodeParser — React/JSX support", () => {
  ...
  test(".tsx files parse without errors", async () => {
    const filepath = await writeFixture("App.tsx", `
  ...
  test("arrow function component detected as 'component' entity", async () => {
    const filepath = await writeFixture("Button.tsx", `
  ...
  test("hook (const useX = () => ...) detected as 'hook' entity", async () => {
    const filepath = await writeFixture("useTheme.tsx", `
  ...
  test("calls inside arrow functions have correct enclosing entity name", async () => {
    const filepath = await writeFixture("Wrapper.tsx", `
    ...
    const calls = relationships.filter(r => r.type === "calls");
    const call = calls.find(r => r.from === "Wrapper" && r.to === "doSomething");
    expect(call).toBeDefined();
  });

  test("nested JSX — both parent and child detected", async () => {
    const filepath = await writeFixture("Layout.tsx", `
    ...
    const calls = relationships.filter(r => r.type === "calls" && r.from === "Layout");
  ...
  test("intrinsic HTML elements excluded from relationships", async () => {
    const filepath = await writeFixture("Plain.tsx", `
  ...
  test("member expression JSX (e.g. motion.div) handled gracefully", async () => {
    const filepath = await writeFixture("Animated.tsx", `
  ...
  test("full React fixture — entities and relationships", async () => {
    const filepath = await writeFixture("Screen.tsx", `
    ...
    // Relationships — calls
    const calls = relationships.filter(r => r.type === "calls");
    expect(calls.find(r => r.from === "App" && r.to === "Button")).toBeDefined();
    expect(calls.find(r => r.from === "Screen" && r.to === "useTheme")).toBeDefined();
    expect(calls.find(r => r.from === "Screen" && r.to === "App")).toBeDefined();
  });

  test("existing call graph for named functions still works", async () => {
    const filepath = await writeFixture("plain.ts", `
