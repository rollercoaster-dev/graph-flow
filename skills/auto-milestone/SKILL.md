...
2. **Call the tool** — Use `a-from-milestone` with the milestone number.
3. **Report results** — Show: goal ID, plan ID, number of issues imported, and a summary.
4. **Suggest next steps:**
   - "Use `p-progress` to check progress at any time"
   - "Use `/work-on-issue` to start working on a specific issue"
   - "Use `p-steps` to adjust dependencies or wave groupings"
