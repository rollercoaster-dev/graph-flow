# /visual-work-on-issue <issue-number> [flags]

Wraps `/work-on-issue` with before/after browser screenshots. The inner workflow is unchanged — this command bookends it with visual verification.

**Flags:** All `/work-on-issue` flags pass through (`--skip-review`).
...
```bash
/work-on-issue <N> [flags]
```

This executes the full gated workflow (setup, research with gates, implement with per-commit approval, review, finalize) **completely unchanged**. It produces a PR.
...
---
*Screenshots taken by visual-work-on-issue wrapper*
EOF
)"
```
...
```text
visual-work-on-issue <N>
├─ Phase 0: Generate sitemap → match issue → screenshot BEFORE
├─ Phases 1-5: /work-on-issue <N> (unchanged, gated)
