...
3. **If parallel mode (`--parallel N > 1`)**: No manual worktree setup needed. Workers are spawned with `isolation: "worktree"` on the Task call (see Phase 2 in [docs/multi-issue-workflow.md](../docs/multi-issue-workflow.md)). Claude Code creates and cleans up worktrees automatically.
